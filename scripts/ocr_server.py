...
def _dedupe_bboxes(blk_list, iou_threshold=0.3):
    ...
    for blk in sorted_blks:
        ...
        for existing in kept:
            ebox = tuple(float(v) for v in existing.xyxy)
            if _bbox_iou(box, ebox) > iou_threshold:
                overlaps = True
                break
        if not overlaps:
            kept.append(blk)
...
def ocr_bubbles(img_cv):
    ...
    # 중복 bbox 제거 (CTD가 같은 영역을 여러 번 검출하는 경우 방지)
    ...
    for blk in blk_list:
        ...
        crops.append((pil, (x1, y1, x2, y2)))

    # OCR 병렬 실행 (manga_ocr는 GIL 밖에서 PyTorch 연산하므로 threading 유효)
    t1 = time.time()
    if crops:
        with ThreadPoolExecutor(max_workers=4) as ex:
            texts = list(ex.map(lambda c: mocr(c[0]).strip(), crops))
    else:
        texts = []
    t_ocr = time.time() - t1
    ...
    results = []
    seen = set()
    for (pil, coords), text in zip(crops, texts):
    ...
    print(f"[timing] ctd={t_ctd:.2f}s  ocr={t_ocr:.2f}s  bubbles={len(results)}/{len(crops)}", flush=True)
    return results, mask
...
def translate_openai(bubbles, image_url=None, context='', model=None):
...
def translate_bubbles(bubbles, image_url=None, context='', model=None):
    if OPENAI_API_KEY:
        try:
            return translate_openai(bubbles, image_url, context, model)
...
def fit_text(draw, text, box_w, box_h, font_path):
    """한국어는 항상 가로쓰기 + 줄바꿈. 들어가는 최대 폰트 크기를 선택."""
    for size in range(24, 7, -1):
        ...
        if len(lines) * (size + 3) <= box_h:
            return font, "\n".join(lines), size
    # 최소 크기로도 안 들어가면 일단 8px로 그리기 (넘치는 건 감수)
...
def wrap_text(draw, text, font, max_width):
    """공백 단위로 줄바꿈. 한 단어가 max_width보다 길면 어쩔 수 없이 문자 단위로 쪼갬."""
    def tw(s):
        bbox = draw.textbbox((0, 0), s, font=font)
        return bbox[2] - bbox[0]

    def break_long_word(word):
        """한 단어가 max_width보다 길 때 문자 단위로 쪼개서 반환."""
        chunks, buf = [], ""
        for ch in word:
            test = buf + ch
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif self.path == "/translate-render":
    ...
    def _handle_translate(self, body):
        ...
        try:
            ...
            if not bubbles:
                ...
                return
            translated = translate_bubbles(bubbles, image_url, context, model)
            self._json(200, {
                "bubbles": translated,
    ...
    def _handle_translate_render(self, body):
        """OCR + 번역 + inpainting + 렌더링을 한 번에 처리. 결과는 base64 JPEG."""
        import time
        image_url = body.get("imageUrl", "")
        context = body.get("context", "")
        model = body.get("model", "")
        ...
        try:
            ...
            if img_cv is None:
                self._json(500, {"error": "failed to decode image"})
                return
            bubbles, mask = ocr_bubbles(img_cv)
            if not bubbles:
            ...
            print(f"[timing] download={t_dl:.2f}s  translate={t_tr:.2f}s  render={t_render:.2f}s  total={time.time()-t0:.2f}s", flush=True)
            self._json(200, {
                "bubbles": translated,
